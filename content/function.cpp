...
std::string FunctionCustom::run(std::vector <std::string>& arguments) {
    ...
    for (unsigned int n = 0; n < variables.size(); n++) {
        if (variables[n].type == Variable::typeNum) {
            if (!IsConvertibleToNum(arguments[n])){
                InterpreterException("Invalid function arguments!");
                functionStack.pop();
                return "0";
            }
            functionStack.top().variables[variables[n].name] = std::unique_ptr <Variable> (new VariableNum(std::stold(arguments[n])));
        }
...
void ParseNewFunction(std::pair<unsigned int, unsigned int> range){
    ...
    else if (parsedLine[range.first] == "num"){
        newFunctionReturnType = Variable::typeNum;
    }
    ...
    for (unsigned int n = range.first + 3; n < range.second; n++) {
        if (parsedLine[n] == ")" or parsedLine[n] == ",") {
            ...
            if (variableRange.second - variableRange. first == 2){
                // <type> <token>
                if (parsedLine[variableRange.first] == "num"){
                    newFunctionVariables.emplace_back(Variable::typeNum, parsedLine[variableRange.first + 1], false);
                }
                ...
                if (parsedLine[variableRange.first + 1] == "num"){
                    newFunctionVariables.emplace_back(Variable::typeNum, parsedLine[variableRange.first + 2], true);
                }
